...
Section default_order.
  ...
  Proof.
    ...
    split; intros [n [d d_nonzero]]; exists n; exists d.
     now rewrite <-E, <-E'.
    now rewrite E, E'.
  Qed.

  Instance: Reflexive rationals_le.
  Proof. intro. exists (0:nat); exists (0:nat). rewrite preserves_0. ring. Qed.
  ...
  Proof with auto.
    ...
    exists (n1 * d2 + n2 * d1) ; exists (d1 * d2).
    rewrite B2, B1.
  ...
  Proof.
    apply from_ring_order.
     repeat (split; try apply _).
     intros x y [n [d E]]. exists n ; exists d. rewrite E. ring.
    intros x y [n1 [d1 E1]] [n2 [d2 E2]].
    exists (n1 * n2) ; exists (d1 * d2).
